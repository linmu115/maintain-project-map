...
分类不必按记录 kind 平铺。组合项目可以在 records/modules/ 下按职责嵌套模块目录，文件继续带独立记录身份；也可绑定成员仓库中的现有章节。模块层级、提供方合同与消费方接入说明的组织见[组合项目与扩展接口](composite-projects.md)。文件夹不自动成为另一项目或阅读器目录树。
