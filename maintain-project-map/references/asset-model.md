...
| history | 有明确来源范围的任务过程，连接问题、尝试、反馈、转折与结果 |
| experience | 属于一项任务、可独立查找的排错、改进、验证或纠偏经验；分类只提供多种入口 |
...
`history` 和 `experience` 的来源、分类及按需读取见 [开发历程](development-history.md)。经验引用已有实现与验证记录，不替代它们，不重复保存原始事件。
